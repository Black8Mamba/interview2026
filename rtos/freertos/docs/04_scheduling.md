...
在 FreeRTOS 中，当一个任务调用 `vTaskDelay(10)` 延时 10ms 后，它何时被判断时间到了并重新就绪？这个判断过程的时间复杂度是多少？

**关键问题：**
- 如果有 N 个任务同时延时，调度器需要花多少时间判断哪些任务到期了？
- 遍历所有延时任务 → O(n)？还是更优的算法？

### 4.3.2 延时任务的核心：两个延时链表

FreeRTOS 使用 **两个延时链表** 交替工作，这是实现高效唤醒的关键设计：

```c
// tasks.c 中的延时链表定义
typedef struct xLIST {
    volatile ListItem_t *pxIndex;
    UBaseType_t uxNumberOfItems;
    MiniListItem_t xListEnd;
} List_t;

// 两个延时链表
static List_t xDelayedTaskList1;   // 链表1
static List_t xDelayedTaskList2;    // 链表2
static List_t *pxDelayedTaskList;   // 指向当前使用的链表
static List_t *pxOverflowDelayedTaskList;  // 处理跨 Tick 溢出的延时

// 下一个任务的到期时间
static volatile TickType_t xNextTaskUnblockTime;
```

**设计思想：用空间换时间，避免每次 Tick 移动所有任务**
...
│  │                                                              │    │
│  │  xDelayedTaskList1 (当前使用):                              │    │
│  │  [TaskA: wake@N+5] → [TaskB: wake@N+10] → [TaskC: wake@N+20]│   │
│  │                                                              │    │
│  │  xDelayedTaskList2 (备用):                                  │    │
│  │  [TaskX: wake@N+100] → END                                  │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                      │
│  当 xDelayedTaskList1 的所有任务都被唤醒后：                          │
│  pxDelayedTaskList 指针切换，指向 xDelayedTaskList2                  │
│  旧的 xDelayedTaskList1 成为新的备用链表                             │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

### 4.3.3 延时链表排序：按到期时间升序

**关键设计：延时链表按 `xTimeToWake` 升序排列**

```
延时链表排序示意图：

[TaskA: wake@7] → [TaskB: wake@10] → [TaskC: wake@15] → END
      ↑                     ↑                    ↑
    最早到期              中间到期              最晚到期

好处：一旦遇到未到期的任务，后面的都不用检查！
```

### 4.3.4 xTaskIncrementTick：每个 Tick 的唤醒检查

**这是理解延时任务就绪判断的核心函数**

```c
// xTaskIncrementTick - 每次 SysTick 中断调用
BaseType_t xTaskIncrementTick(TickType_t xTicks) {
    BaseType_t xSwitchRequired = pdFALSE;
    ...
    if (uxSchedulerSuspended == pdFALSE) {
        ...
        if (xConstTickCount >= xNextTaskUnblockTime) {
            ...
            for (;;) {
                ...
                // 比较：当前 Tick >= 任务的到期时间？
                if (pxTCB->xTimeToWake > xConstTickCount) {
                    // 还没到期，后面的任务更晚，都不用检查了
                    // 更新下一个到期时间，退出循环
                    xNextTaskUnblockTime = pxTCB->xTimeToWake;
                    break;
                }
                ...
                if (!listLIST_IS_EMPTY(pxDelayedTaskList)) {
                    Task_t *pxNextTCB = listGET_OWNER_OF_HEAD_ENTRY(pxDelayedTaskList);
                    xNextTaskUnblockTime = pxNextTCB->xTimeToWake;
                }
            }
        }
...
```
假设当前 Tick = 7，有 3 个延时任务在链表中：

延时链表（已按到期时间排序）:
[TaskA: wake@7] → [TaskB: wake@10] → [TaskC: wake@15]

─────────────────────────────────────────────────────────────

Tick = 7 时，SysTick 中断触发 xTaskIncrementTick()：

步骤1：Tick 计数 +1
    uxTickCount = 7

步骤2：检查是否到达到期时间
    xNextTaskUnblockTime = 7
...
步骤5：TaskA 从延时链表移除，加入就绪队列
    prvAddTaskToReadyQueue(TaskA)  // O(1) 操作

步骤6：继续检查下一个
    pxTCB = TaskB
    if (7 >= 10)? 否，TaskB 还没到期
    → 停止检查（链表有序，后面的更晚）

步骤7：更新 xNextTaskUnblockTime = 10

步骤8：返回 pdTRUE，触发 PendSV 切换到 TaskA
```
...
| 操作 | 时间复杂度 | 原因 |
|------|----------|------|
| 判断首个任务是否到期 | **O(1)** | 只看链表头 |
| 到期任务移入就绪队列 | **O(1)** | 直接从链表头移除，插入就绪队列 |
| 查找下一个要检查的任务 | **O(1)** | 链表头就是下一个 |
| 新任务加入延时链表 | **O(n)** | 遍历找到正确的排序位置 |

```
为什么这样设计？

优点：
1. 唤醒判断 O(1) — 每次 Tick 只看头节点
2. 链表有序 — 一旦遇到未到期，后面的都不用查
3. 两个链表交替 — 避免每次 Tick 移动所有任务

缺点：
4. 插入新延时任务是 O(n)
5. 但延时插入是"写操作"，唤醒检查是"读操作"

这是经典的"读多写少"优化策略！
嵌入式系统中，任务延时到期是高频事件，任务进入延时相对低频。
```

### 4.3.7 vTaskDelay 内部实现流程

```c
// vTaskDelay 内部实现
void vTaskDelay(TickType_t xTicksToDelay) {
    Task_t *pxTask = pxCurrentTCB;

    // 计算任务的绝对到期时间
    // xTicksToDelay：延时的 Tick 数
    // uxTickCount：当前 Tick 计数
    // xTimeToWake：到期时的 Tick 计数
    pxTask->xTimeToWake = uxTickCount + xTicksToDelay;

    // 任务状态：Running → Blocked
    listREMOVE_ITEM(&pxTask->xStateListItem);

    // 插入延时链表（按 xTimeToWake 升序插入）
    vTaskPlaceOnUnorderedEventList(
        &pxTask->xEventListItem,
        pxTask->xTimeToWake | 0x80000000UL  // 标记为延时类型
    );
...
### 4.3.8 延时链表插入：O(n) 的代价

```c
// vTaskPlaceOnUnorderedEventList - 延时链表插入
// 需要遍历找到正确的插入位置
...
void vTaskPlaceOnUnorderedEventList(
    ...
    // 从链表头部开始遍历
    while (pxIndex->pxNext != (ListItem_t *)pxList) {
        if (xItemValue < pxIndex->pxNext->xItemValue) {
            // 找到插入位置：在 pxIndex 之后
            break;
        }
    ...
    // 插入链表
    pxEventListItem->pxNext = pxIndex->pxNext;
    pxEventListItem->pxPrevious = pxIndex;
    pxIndex->pxNext = pxEventListItem;
    (pxEventListItem->pxNext)->pxPrevious = pxEventListItem;

    // 更新 xNextTaskUnblockTime
    if (xItemValue < xNextTaskUnblockTime) {
...
时间线：
Tick 0:  TaskA 调用 vTaskDelay(10) → 插入延时链表，wake@10
Tick 0:  TaskB 调用 vTaskDelay(5)  → 插入延时链表，wake@5
Tick 0:  TaskC 调用 vTaskDelay(15) → 插入延时链表，wake@15

延时链表（Tick=0时）：
[TaskB: wake@5] → [TaskA: wake@10] → [TaskC: wake@15]
                   ↑ xNextTaskUnblockTime = 5

─────────────────────────────────────────────────────────────

Tick 5: SysTick 中断，xTaskIncrementTick()

检查：5 >= 5? ✓ TaskB 到期
    → TaskB 移入就绪队列

检查：5 >= 10? 否，TaskA 还没到期
    → 停止，xNextTaskUnblockTime = 10

触发 PendSV，切换到 TaskB（如果 TaskB 优先级更高）

─────────────────────────────────────────────────────────────

Tick 10: SysTick 中断，xTaskIncrementTick()

检查：10 >= 10? ✓ TaskA 到期
    → TaskA 移入就绪队列

检查：10 >= 15? 否，TaskC 还没到期
    → 停止，xNextTaskUnblockTime = 15

触发 PendSV，切换到 TaskA

─────────────────────────────────────────────────────────────

Tick 15: SysTick 中断，xTaskIncrementTick()
...
│  就绪队列插入（任务创建）    O(1)         低                  │
│  就绪队列出队（任务切换）    O(1)         高                  │
│  选择最高优先级任务         O(1)         高（每次切换）        │
│  延时任务唤醒检查          O(1)         极高（每Tick）        │
│  延时任务插入             O(n)         中等（每次延时）        │
│  同优先级时间片轮转        O(n)         高（但n通常很小）      │
│                                                              │
└─────────────────────────────────────────────────────────────┘

设计决策：
1. 读操作（唤醒检查）优化到 O(1) — 每 Tick 都在发生
2. 写操作（延时插入）允许 O(n) — 发生频率较低
3. 链表有序使得唤醒检查可以提前终止
