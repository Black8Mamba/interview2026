...
+=========================================================================+
    ...
    +---------------+     +---------------+     +---------------+
    | 设置中断分组   |     | 设置优先级     |     | 设置触发类型  |
    | group参数     |     | priority参数  |     | signal参数   |
    | 0=FIQ, 1=IRQ |     | 0-31 (越小越高)|     | 边沿/电平    |
    +---------------+     +---------------+     +---------------+
            |                     |                     |
            +---------------------+---------------------+
                                  |
...
+=========================================================================+
  ...
  +-------------------------------------------------------+
    ...
    v             v             v             v
+-------+   +-------+   +-------+   +-------+
|设置分组|   |设置优先级|   |设置路由|   |设置触发|
|IGROUPR|   |IPRIORITYR|   |IROUTER|   |ICFGR  |
+-------+   +-------+   +-------+   +-------+
...
+=========================================================================+
  ...
  |  自动判断中断类型:                                     |
  |                                                       |
  |   if (SGI)   gic_enable_sgi(int_id, core);        |
  |   if (PPI)   gic_enable_ppi(int_id, core);        |
  |   if (SPI)   gic_enable_spi(int_id);               |
  |                                                       |
  +-------------------------------------------------------+
                    |
    +-------------+-------------+
