...
+=========================================================================+
    ...
    |   |   2      |  Cluster 0, Core 2                            |  |
    |   |   3      |  Cluster 0, Core 3                            |  |
    |   |  0x10000 |  Cluster 1, Core 0                            |  |
    |   |  0x10001 |  Cluster 1, Core 1                            |  |
    |   |  0x20000 |  Cluster 2, Core 0                            |  |
    |   +----------+------------------------------------------------+  |
    +------------------------------------------------------------------+
...
+=========================================================================+
    ...
    +--------+        +--------+        +--------+
    | Core 0 |        |        |        | Core 1 |
    | (Cluster0)      |  IBCM  |        | (Cluster1)
    |        |------->|        |<-------|
    +--------+        +--------+        +--------+

    IBCM 消息格式:
    +------------------------------------------------------------------+
    |  bit[11:8]   bit[7:4]    bit[3:1]    bit[0]                   |
    |  Reserved   Cluster    Core      Reserved                     |
    +------------------------------------------------------------------+

    限制:
    +------------------------------------------------------------------+
    |  Cluster类型      |  最大SGI数量  |  说明                     |
    +-------------------|---------------|---------------------------|
    |  Cluster 0-2     |     8         |  每个核心最多8个SGI       |
    |  Cluster 3 (外设) |     4         |  DME/DSPH最多4个SGI     |
    +------------------------------------------------------------------+
...
+=========================================================================+
    ...
    |   根据负载动态调整中断目标核心                          |
|                                                        |
|   if (core0_load > threshold) {                      |
|       // 将新中断路由到其他核心                         |
|       irq_route_change(int_id, new_core);           |
|   }                                                  |
|                                                        |
+-------------------------------------------------------+
